...
class TopSelectionScreen extends StatelessWidget {
  const TopSelectionScreen({super.key});
            ...
            child: Column(
              children: [
                ...
                _TopList(
                  ...
                  selectionList: topSelectionList,
                ),
                BackNextButtons(
                  onNextPressed: () async {
                    print('Final selection list = $topSelectionList');
                    controller.selectionList = topSelectionList;
                    //api request하기
                    List<RecommendationModel> recommendationModelList = await controller.getSelectionBasedRecommendation();
                    print('top---------------------------------');
                    print(recommendationModelList);
                    Get.to(
  ...
  List<ItemModel> pickClothesListRandomly() {
    ...
    for (int i = 0; i < 18; i++) {
      int randomIndex = random.nextInt(topClothesNameList.length);
      final selectedClothesName = topClothesNameList[randomIndex];
